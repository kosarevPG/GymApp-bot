...
if [ -z "$SPREADSHEET_ID" ] || [ -z "$GOOGLE_CREDENTIALS_JSON" ] || [ -z "$AUTH_TOKEN" ]; then
  echo "Для деплоя обязательны SPREADSHEET_ID, GOOGLE_CREDENTIALS_JSON и AUTH_TOKEN."
  exit 1
fi

cd "$(dirname "$0")/.."
ARCHIVE="$(pwd)/backend-deploy.zip"
BACK_ABS="$(cd "$BACK_DIR" 2>/dev/null && pwd || true)"
if [ -z "$BACK_ABS" ] || [ ! -d "$BACK_ABS" ]; then
  echo "Папка $BACK_DIR не найдена"
  exit 1
fi
(cd "$BACK_ABS" && zip -r "$ARCHIVE" . -x "*.pyc" -x "__pycache__/*")

ENV_ARGS=()
[ -n "$SPREADSHEET_ID" ] && ENV_ARGS+=(--environment "SPREADSHEET_ID=$SPREADSHEET_ID")
[ -n "$GOOGLE_CREDENTIALS_JSON" ] && ENV_ARGS+=(--environment "GOOGLE_CREDENTIALS_JSON=$GOOGLE_CREDENTIALS_JSON")
